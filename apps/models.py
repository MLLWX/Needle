...
class ConvBN(ndl.nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride, device, dtype="float32"
    ):
        self.conv2d = ndl.nn.Conv(
...
class ResNet9(ndl.nn.Module):
    def __init__(self, device=None, dtype="float32"):
        ...
        self.linear = ndl.nn.Linear(128, 128, device=device, dtype=dtype)
        self.relu = ndl.nn.ReLU()
        self.out = ndl.nn.Linear(128, 10, device=device, dtype=dtype)
        ### END YOUR SOLUTION
    ...
    def forward(self, x: ndl.Tensor):
        ...
        x = self.block_3(x)
        x = self.block_4(x)
        x = self.relu(self.linear(self.flatten(x)))
        out = self.out(x)
        return out
        ### END YOUR SOLUTION
...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, h=None):
        ...
        seq_len, bs = x.shape
        x = self.embedding(x)
        H, h = self.seq_model(x, h)
        out = self.linear(H.reshape((seq_len*bs, self.hidden_size)))
        return out, h
        ### END YOUR SOLUTION
