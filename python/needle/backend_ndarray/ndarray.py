...
class NDArray:
    ...
    def make(shape, strides=None, device=None, handle=None, offset=0):
        ...
        array."""
        assert np.all([isinstance(s, int) for s in tuple(shape)]).item(), "shape must be Integer"
        if strides is not None:
            assert np.all([isinstance(s, int) for s in tuple(strides)]).item(), "strides must be Integer"
    ...
    def squeeze(self, axis=None):
        if axis is None:
            ...
            return self.compact().reshape(tuple(shape))
        elif self.shape[axis] == 1:
    ...
    def permute(self, new_axes):
        ...
        if sorted(new_axes) != list(range(self.ndim)):
            raise ValueError("new axes must be permute order of the dimensions")
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert self.ndim == len(new_shape), "new NDArray ndim shoud equal to the origin one"
        
        new_strides = list(self._strides)
        for i, (dim_new, dim) in enumerate(zip(new_shape, self._shape)):
            if (dim_new != dim) and (dim != 1):
                raise AssertionError("new_shape[i] should equal to shape[i] " 
                                     "for all i where shape[i] != 1")
            elif (dim_new != dim) and (dim == 1):
                new_strides[i] = 0

        out = self.make(shape=tuple(new_shape), 
    ...
    def __getitem__(self, idxs):
        ...
        for i, s in enumerate(idxs):
            dim = math.ceil((s.stop - s.start) / s.step)
            new_shape.append(dim)
            new_strides.append(old_strides[i]*s.step)
            new_offset += s.start * old_strides[i]
    ...
    def flip(self, axes):
        ...
        return self.make(self.shape, new_stride, self.device, self._handle, new_offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i, (l, r) in enumerate(axes):
            idx.append(slice(l, l+shape[i]))
            shape[i] += l + r
        out = full(shape=tuple(shape), fill_value=0, dtype=self.dtype, device=self.device)
        out[tuple(idx)] = self
        return out
        ### END YOUR SOLUTION
