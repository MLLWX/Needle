...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        if self.causal:
            mask = self.create_causal_mask(queries_len, keys_values_len, q.device).broadcast_to(
                (batch_size, num_head, queries_len, keys_values_len)
            )
        else:
            mask = 0.0
        probs = self.softmax(self.matmul(q, k) / (q_dim**0.5) + mask)
        probs = self.dropout(probs)
        result = self.matmul(probs, ops.transpose(v, (2, 3)))
        ### END YOUR SOLUTION
...
class AttentionLayer(Module):
    ...
    def forward(
        self,
        q: Tensor, k: Optional[Tensor] = None, v: Optional[Tensor] = None,
    ):
        """
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        ### BEGIN YOUR SOLUTION
        attn = AttentionLayer(q_features, num_head, dim_head, out_features=q_features, 
                              dropout=dropout, causal=causal, device=device, dtype=dtype)
        sequential_1 = Sequential(attn, Dropout(dropout))
        self.block_1 = Residual(sequential_1)

        sequential_2 = Sequential(LayerNorm1d(q_features, device=device, dtype=dtype),
                                  Linear(q_features, hidden_size, device=device, dtype=dtype),
                                  ReLU(),
                                  Dropout(dropout),
                                  Linear(hidden_size, q_features, device=device, dtype=dtype),
                                  Dropout(dropout))
        self.block_2 = Residual(sequential_2)
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        ### BEGIN YOUR SOLUTION
        self.pos_embedding = Embedding(sequence_len, embedding_dim=embedding_size, device=device, dtype=dtype)
    ...
    def forward(
        ...
        pos = Tensor(np.arange(seq_len), device=self.device, requires_grad=False)
        pos_embedding = self.pos_embedding(pos.reshape((1, seq_len))).broadcast_to((bs, seq_len, x_dim))
        x = self.transformer_layers(x + pos_embedding)
        ### END YOUR SOLUTION
