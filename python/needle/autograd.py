...
class Tensor(Value):
    ...
    def __rtruediv__(self, other):
        if isinstance(other, Tensor):
            ...
            return needle.ops.PowerScalar(-1)(self) * other
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        if node.requires_grad:
            partial_ajoints = node_to_output_grads_list[node]
            out_grad = sum_node_list(partial_ajoints)
            node.grad = out_grad
            if node.is_leaf():
                continue
            input_partial_ajoints = node.op.gradient_as_tuple(out_grad, node)
            for input_node, partial_ajoint in zip(node.inputs, input_partial_ajoints):
...
def topo_sort_dfs(node: Value, visited: Set, topo_order: List[Value]):
    ...
    for input_value in node.inputs:
        if id(input_value) not in visited:
            topo_sort_dfs(input_value, visited, topo_order)
